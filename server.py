...
from file_handler import search_file
...
class UDPServer:
    ...
    def __init__(self, ip, port, directory, flask_port):
        self.ip = ip
        self.port = int(port)
    ...
    def _start(self):
        ...
        while True:
            ...
            executor.submit(self._process_request, msg=msg, addr=addr, dir=self.dir, flask_ip=self.ip,
                            flask_port=self.flask_port)

    def _process_request(self, msg, addr, directory, flask_ip, flask_port):
        ...
        if tokens[1] == "JOIN":
            ...
            hops = int(tokens[5])
            files_found, file_names = search_file(directory, tokens[4])
